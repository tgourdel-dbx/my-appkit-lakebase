...
// Idempotent so a pre-existing todos table gains the due_date column. The
// authoritative schema change lives in migrations/ (applied to production on
// merge); this keeps local dev working before `npm run migrate` has run.
    ...
    app.get('/api/lakebase/todos', async (_req, res) => {
      try {
        const result = await appkit.lakebase.query(
          'SELECT id, title, completed, url, due_date::text AS due_date, created_at FROM app.todos ORDER BY created_at DESC'
        );
