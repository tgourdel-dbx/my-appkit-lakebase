...
// Today as a local YYYY-MM-DD string, for comparing against due dates.
...
function formatDueDate(due: string): string {
  // due is a YYYY-MM-DD calendar date; parse as local time to avoid TZ shifts.
...
export function LakebasePage() {
  ...
  const addTodo = async (e: React.FormEvent) => {
    ...
    const due_date = newDueDate.trim();
    ...
    try {
      const res = await fetch('/api/lakebase/todos', {
        ...
        body: JSON.stringify({ title, ...(url ? { url } : {}), ...(due_date ? { due_date } : {}) }),
      });
